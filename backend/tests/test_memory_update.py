...
def extraction(*, review_required=False):
    return StructuredActivityExtraction(
        ...
        requirements=[RequirementCandidate(title="Support SSO", description="SAML SSO is required", confidence=0.9)],
        tasks=[TaskCandidate(title="Send POC plan", owner_type=OwnerType.SELF, confidence=0.8)],
        decisions=[DecisionCandidate(title="Run a POC", description="Customer agreed to run a POC", confidence=0.85)],
        risks=[RiskCandidate(title="Environment unavailable", description="Test environment is not ready", severity=RiskSeverity.HIGH, confidence=0.7)],
